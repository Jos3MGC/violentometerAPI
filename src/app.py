...
def handle_form():
    ...
    try:
        ...
        cur.execute(
            "INSERT INTO formularioayuda (names, surnames, eventZone, email, phone, idPlatform, age) VALUES (%s, %s, %s, %s, %s, %s, %s)",
        ...
        return (
            jsonify({"code": 1, "message": "¡Exito!, denuncia subida correctamente."}),
            200,
        )
        ...
        return (
            jsonify(
                {
                    "code": 0,
                    "message": "¡Ups!, hubo un error al subir la denuncia.",
                    "error": str(e),
                }
            ),
            500,
...
def enviar_correo_handler():
    ...
    cuerpo = render_template(
        "correo.html", nombres=names, apellidos=surnames
    )
    ...
    try:
        ...
        return (
            jsonify(
                {
                    "status": 1,
                    "message": "Correo de confirmacion enviado correctamente.",
                }
            ),
            200,
        )
        ...
        return (
            jsonify({"status": 0, "message": "Error al enviar el correo: " + str(e)}),
            400,
        )
